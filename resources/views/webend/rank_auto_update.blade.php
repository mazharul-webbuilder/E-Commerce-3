...
<section class="w-full bg-white p-3 mt-5">
    <div class="container px-2 mx-auto xl:px-5">
        ...
        <!-- back button end -->
        @if(Request::routeIs('coins_auto_update.edit'))
            <section>
            ...
            <!-- Rank form start -->
                <div class="border border-[#8e0789] rounded-md mt-10">
                    <div class="bg-[#8e0789] overflow-hidden w-full px-0">
                        <h2 class="text-2xl font-bold py-2 text-white pl-3">Settings Update of {{ $coinRank->title  }} </h2>
                    </div>
                    <!-- rank form start -->
                    <form action="{{ route('coins_auto_update.update',$coinRank->id) }}" method="POST">
                        @csrf
                        <div class="flex flex-col gap-4 p-4 mt-3">
                            <div class="flex flex-col md:flex-row justify-between gap-3">
                                <div class="w-full">
                                    <div class="w-full">
                                        <h4 class="mb-2 font-medium text-zinc-700">{{$needed_member}} Needed</h4>
                                        <input  name="member" required value="{{ $coinRank->member }}" class="w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none" type="number" step="0.01" >
                                    </div>
                                </div>
        ...
        <div class="border border-[#8e0789] rounded-md mt-10">
            <div class="bg-[#8e0789] overflow-hidden w-full px-0 flex items-center">
                <h2 class="text-2xl font-bold py-2 text-white pl-3">Rank update individual member needed</h2>
            </div>
            <div class="py-2 px-1 mt-3" style="overflow-x: auto;">
                <table class="text-sm text-left text-white border-l border-r" id="dataTableAuthor" style=" width: 100%;">
                    ...
                    <tbody>
                    @foreach($rank_auto_update as $data)
                    ...
                    <tr class=" bg-white  dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        @else
                            ...
                            <td class="whitespace-nowrap space-x-1 text-center px-2 flex items-center justify-center">
                                <a href="{{ route('coins_auto_update.edit', $data->id) }}" class="text-white bg-sky-400 hover:bg-sky-500 transition-all ease-in-out font-medium rounded-md text-sm inline-flex items-center px-5 py-2 text-center">
