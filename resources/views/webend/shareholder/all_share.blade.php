        ...
        <div class="container px-2 mx-auto xl:px-5">
            ...
            <div class="border border-[#8e0789] rounded-md mt-10">
                <div class="bg-[#8e0789] overflow-hidden w-full px-0 py-3 flex items-center">
                    <h2 class="text-2xl font-bold py-2 text-white pl-3">Share Owner List</h2>
                    ...
                    <table class="text-sm text-left text-white border-l border-r" id="dataTableAuthor"
                        ...
                        @foreach ($datas as $data)
                            <tr
                                ...
                                <td
                                    class="px-2 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap border-r text-center">
                                    {{ $data->share_owner->name ?? 'Unavailable' }}
                                </td>
                                <td class="px-2 py-4 text-black border-r text-center">
                                    {{ $data->share_purchase_price }}
                                </td>
                                <td class="px-2 py-4 text-black border-r text-center">
                                    {{ $data->share_number }}
                                </td>
                                ...
                                <td class="whitespace-nowrap space-x-1 text-center px-2 flex items-center justify-center">
                                    ...
                                    <input type="text" name="add_parent" class="add_parent add_message{{$data->id}} w-full h-12 p-1 border border-gray-300 rounded-md focus:outline-none text-center text-zinc-700"
                                           data-action="{{route('shareholder.add_parent')}}" share_holder_id="{{$data->id}}" value="{{get_user($data->parent_id)->playerid ?? ""}}">
...
@section('extra_js')
    ...
    <script !src="">
        $(document).ready(function() {
            $("body").on('keyup','.add_parent',function (){
                let parent_number=$(this).val();
                let share_holder_id=$(this).attr('share_holder_id');
                if (parent_number !==""){
                    $.ajax({
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                        method:"post",
                        url:$(this).attr('data-action'),
                        data:{parent_number:parent_number,share_holder_id:share_holder_id},
                        success:function (response){

                            if (response.status==200){

                                $(".add_message"+share_holder_id).addClass('bg-green-100').removeClass('bg-red-100')
                            }else{

                                $(".add_message"+share_holder_id).addClass('bg-red-100').removeClass('bg-green-100')
                            }
                        }
                    })
                }else{
