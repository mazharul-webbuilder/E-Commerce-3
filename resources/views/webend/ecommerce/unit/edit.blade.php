        ...
        <div class="container px-2 mx-auto xl:px-5">
                ...
                <ol class="flex items-center space-x-1 md:space-x-3 flex-wrap">
                    ...
                    <li aria-current="page" class="flex items-center">
                        ...
                        <div class="flex items-center">
                            <span class="ml-1 font-medium text-gray-500 md:ml-2 dark:text-gray-400">Product unit Update</span>
                        </div>
                    </li>
                </ol>
            </div>
            ...
            <div class="border border-[#8e0789] rounded-md mt-10">
                <div class="bg-[#8e0789] overflow-hidden w-full px-0">
                    <h2 class="text-2xl font-bold py-2 text-white pl-3">Unit Update</h2>
                </div>
                <!-- Category form start -->
                <form id="submit_form" data-action="{{ route('unit.update') }}" method="POST">
                    @csrf
                    <div class="flex flex-col gap-4 p-4 mt-3">
                        <div class="flex flex-col md:flex-row justify-between gap-3">
                            ...
                            <div class="w-full flex flex-col lg:flex-row gap-4">
                                <div class="w-full">
                                    <h4 class="mb-2 font-medium text-zinc-700">Unit Name</h4>
                                    <input placeholder="Enter Unit name" value="{{$unit->name}}" name="name" class="w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none" type="text" >
                                ...
                                <div class="w-full">
                                    ...
                                    <select name="status" class="w-full h-12 pl-4 pr-10 cursor-pointer bg-white border border-gray-300 rounded-md appearance-none text-zinc-700 focus:outline-none">
                                        ...
                                        <option value="0" {{$unit->status==0 ?'selected' : ''}}> In Active</option>
                                    </select>
                                    <span class="status_error text-red-400"></span>
                                </div>
                            </div>
                        </div>
                ...
                $.ajax({
                    ...
                    error:function(response){
                        if (response.status === 422) {
                            ...
                            if (response.responseJSON.errors.hasOwnProperty('status')) {
                                $('.status_error').html(response.responseJSON.errors.status)
                            }else{
                                $('.status_error').html('')
                            }
                        }
