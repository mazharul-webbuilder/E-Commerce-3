        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center justify-start">
                ...
                <div class="text-xl font-bold ml-1 md:ml-16 flex items-center">
                    ...
                    <h1 class="hidden lg:block lg:ml-32 font-semibold text-white">Club Admin </h1>
                </div>
                <!-- logo end -->
            </div>
            <!-- Right elements -->
                ...
                <div class="flex items-center relative">
                    <div class="dropdown relative">
                        ...
                        <button id="dropdownAvatarNameButton" data-dropdown-toggle="dropdownAvatarName" class="flex text-white items-center text-sm font-medium text-gray-900 rounded-full hover:text-blue-600 dark:hover:text-blue-500 md:mr-0 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:text-white" type="button">
                            ...
                            <img class="w-8 h-8 mr-2 rounded-full" src="{{asset('uploads/club_owner/'.Auth::guard('owner')->user()->avatar)}}" alt="user photo">
                        ...
                        <div id="dropdownAvatarName" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
                            <div class="px-4 py-3 text-sm text-gray-900 dark:text-white">
                                <div class="font-medium ">{{Auth::guard('owner')->user()->name}} <br>
                                    Balance  {{Auth::guard('owner')->user()->balance}}</div>
                            </div>
                            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownInformdropdownAvatarNameButtonationButton">
                                <li>
                                    <a href="{{route('club_owner.dashboard')}}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Dashboard</a>
                                </li>

                                <li>
                                    <a href="{{route('club_owner.profile')}}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Profile</a>
                                </li>

                                <li>
                                    <a href="{{route('club_owner.club_info')}}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Club Info</a>
                                </li>

                            </ul>
                            <div class="py-2">
                                <a href="{{ route('club_owner.logout') }}" onclick="event.preventDefault(); document.getElementById('frm-logout').submit();"  class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Sign out</a>

                                <form id="frm-logout" action="{{ route('club_owner.logout') }}" method="POST" style="display: none;">
                                    {{ csrf_field() }}
                                </form>
                            </div>
                        </div>
            ...
            <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                <div class="flex-1 px-3 bg-[#1b1b9b] divide-y space-y-1">
                    <ul class="space-y-2 pb-6" id="sidenavExample">
                        <li>
                            <a href="{{ route('club_owner.dashboard') }}" class="text-lg text-white font-normal rounded-lg {{ Request::routeIs('club_owner.dashboard') ? 'bg-blue-500' : '' }} flex items-center p-2 hover:bg-blue-500 group active:bg-blue-500">
                        ...
                        <li>
                            <a href="{{route('club_owner.tournament_index')}}" class="text-lg text-white font-normal rounded-lg  flex items-center p-2 hover:bg-blue-500 group active:bg-blue-500">
                                <i class="fas fa-trophy text-white group-hover:text-white transition duration-75"></i>
                                <span class="ml-3 group-hover:text-white transition duration-150"> Tournaments</span>
                            </a>
                        </li>

                        <li>
                            <a href="{{route('club_owner.club_member')}}" class="text-lg text-white font-normal rounded-lg  flex items-center p-2 hover:bg-blue-500 group active:bg-blue-500">
                                <i class="fas fa-trophy text-white group-hover:text-white transition duration-75"></i>
                                <span class="ml-3 group-hover:text-white transition duration-150"> Club Member</span>
                            </a>
                        </li>

                        <li>
                            <a href="{{route('club_owner.advertisement_list')}}" class="text-lg text-white font-normal rounded-lg  flex items-center p-2 hover:bg-blue-500 group active:bg-blue-500">
                                <i class="fas fa-trophy text-white group-hover:text-white transition duration-75"></i>
                                <span class="ml-3 group-hover:text-white transition duration-150">Advertisement List</span>
                            </a>
                        </li>
                        <li>
                            <a href="{{route('club_owner.boasting_money')}}" class="text-lg text-white font-normal rounded-lg  flex items-center p-2 hover:bg-blue-500 group active:bg-blue-500">
                                <i class="fas fa-trophy text-white group-hover:text-white transition duration-75"></i>
                                <span class="ml-3 group-hover:text-white transition duration-150">Boasting Money</span>
                            </a>
                        </li>
                        <li>
                            <a href="{{route('club_owner.import_paid_coin')}}" class="text-lg text-white font-normal rounded-lg  flex items-center p-2 hover:bg-blue-500 group active:bg-blue-500">
                                <i class="fas fa-trophy text-white group-hover:text-white transition duration-75"></i>
                                <span class="ml-3 group-hover:text-white transition duration-150">Import Paid Coin</span>
                            </a>
                        </li>
                    </ul>
                </div>
