    ...
    <section class="w-1/2 bg-white p-3 mt-5">
        <div class="container px-2 mx-auto xl:px-5">
            ...
            <div class="border border-[#8e0789] rounded-md mt-10">
                ...
                <!-- Settings form start -->
                <form action="{{ route('settings.3p.update') }}" method="POST">
                    @csrf
                        ...
                        <div class="flex flex-col md:flex-row justify-between gap-3">
                            <div class="w-full">
                                <div class="w-full">
                                    ...
                                    <input  name="first_winner_multiply" value="{{ $free_three_player_settings->first_winner_multiply }}" min="1" class="w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none" type="number" step="any">
                                </div>
                            </div>
                            <div class="w-full">
                                <div class="w-full">
                                    <h4 class="mb-2 font-medium text-zinc-700">Entry Coin Multiply by(For Second Winner)</h4>
                                    <input  name="second_winner_multiply" value="{{ $free_three_player_settings->second_winner_multiply }}" step="0.01" class="w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none" type="number" step="any">
                                </div>
                            </div>
                        </div>
                        <div class="flex flex-col md:flex-row justify-between gap-3">
                            <div class="w-full">
                                <div class="w-full">
                                    ...
                                    <input  name="third_winner_multiply" value="{{ $free_three_player_settings->third_winner_multiply }}" step="0.01" class="w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none" type="number" step="any">
                                </div>
                            </div>
