                        ...
                        @foreach ($datas as $data)
                            <tr
                                ...
                                <td class="px-2 py-4 text-black border-r text-center">
                                    ...
                                    <img class="w-60 h-30 rounded-lg" src="{{asset('uploads/boasting/'.$data->image)}}" alt="" >
                                </td>
                                <td class="px-2 py-4 text-black border-r text-center">
                                    {{ $data->created_at }}
                                </td>
                                ...
                                <td class="px-2 py-4 text-black border-r text-center">
                                    @if($data->status==1 || $data->status==2)
                                        <select name="status" id="manage_boasting" boasting_id="{{$data->id}}" data-action="{{route('admin_club.manage_boasting')}}">
...
@section('extra_js')
    ...
    <script !src="">
        $(function() {
            ...
            $("body").on('change','#manage_boasting',function (){
                let boasting_id=$(this).attr('boasting_id');
                let status=$(this).val();
                $.ajax({
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    method:"post",
                    url:$(this).attr('data-action'),
                    data:{boasting_id:boasting_id,status:status},
