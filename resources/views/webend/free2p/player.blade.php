        ...
        <div class="container px-2 mx-auto xl:px-5">
                ...
                <ol class="flex items-center space-x-1 md:space-x-3 flex-wrap">
                    ...
                    <li class="flex items-center">
                        ...
                        <a href="#" class="flex items-center hover:scale-105">
                            <span class="ml-1 font-medium text-gray-500 md:ml-2 dark:text-gray-400">2 Player</span>
                        </a>
                    </li>
            ...
            <div class="border border-[#8e0789] rounded-md mt-10">
                <div class="bg-[#8e0789] overflow-hidden w-full px-0 flex items-center">
                    <h2 class="text-2xl font-bold py-2 text-white pl-3">2 Player List</h2>
                </div>
                <div class="py-2 px-1 mt-3" style="overflow-x: auto;">
                    <table class="text-sm text-left text-white border-l border-r" id="dataTableAuthor"
                        ...
                        <tbody>
                            @foreach ($free_two_player as $player)
                                <tr
                                    ...
                                    <td class="px-2 py-4 text-black border-r text-center">
                                        {{ \App\Models\User::find($player->winner) ? \App\Models\User::find($player->winner)->name : '' }}
                                    </td>
                                    <td class="px-2 py-4 text-black border-r text-center">
                                        {{ \App\Models\User::find($player->looser) ? \App\Models\User::find($player->looser)->name : '' }}
                                    </td>
                                    <td class="px-2 py-4 text-black border-r text-center">
                                        {{ $player->winner_coin }}
                                    </td>
                                    ...
                                    <td class="px-2 py-4 text-black border-r whitespace-nowrap text-center">
                                        @if ($player->status == 0)
                                            <span class="bg-gray-400 text-white px-2 py-1 rounded"> Running</span>
                                        @else
                                            <span class="bg-green-300 text-white px-2 py-1 rounded"> Complete</span>
                                        @endif
                                    </td>
