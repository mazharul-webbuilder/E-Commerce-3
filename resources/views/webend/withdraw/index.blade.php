                                ...
                                <tr class=" bg-white  dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="px-2 py-4 text-black border-r text-center">
                                        @if($withdraw->status==2)
                                            <input type="text" name="processing_time" withdraw_id="{{$withdraw->id}}"
                                                   class="processing_time w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none"
                                                   data-action="{{route('withdraw.processing_time')}}" value="{{$withdraw->processing_time}}">
                                        @else
                                            <span class="p-4 bg-gray-100">Not Applied</span>
                                        @endif
                                    </td>
                                    <td class="px-2 py-4 text-black border-r text-center">
                                        @if($withdraw->status==1 || $withdraw->status==2)
                                            <select name="status" id="manage_withdrawal" withdraw_id="{{$withdraw->id}}" data-action="{{route('withdraw.manage_withdraw')}}">
                                                <option value="1" {{$withdraw->status ==1 ? 'selected' : ''}}>Pending</option>
                                                <option value="2" {{$withdraw->status ==2 ? 'selected' : ''}}>Processing</option>
                                                <option value="3" {{$withdraw->status ==3 ? 'selected' : ''}}>Accept</option>
                                                <option value="4" {{$withdraw->status ==4 ? 'selected' : ''}}>Reject</option>
                                            </select>
                                        @else
