        ...
        <div class="container px-2 mx-auto xl:px-5">
            ...
            <div class="border border-[#8e0789] rounded-md mt-10">
                <div class="bg-[#8e0789] overflow-hidden w-full px-0 py-3 flex items-center">
                    <h2 class="text-2xl font-bold py-2 text-white pl-3">Friend List</h2>
                ...
                <div class="py-2 px-1 mt-3" style="overflow-x: auto;">
                    <table class="text-sm text-left text-white border-l border-r" id="dataTableAuthor" style=" width: 100%;">
                        ...
                        @foreach($lists as $list)
                            @if($list->user_id_one == $user['id'])
                        <tr class=" bg-white  dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="px-2 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap border-r text-center">
                              {{ $loop->iteration }}
                            </td>
                            <td class="px-2 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap border-r text-center">
                                {{ player_id($list->user_id_two) }}
                            </td>

                            <td class="px-2 py-4 text-black border-r text-center">
                              {{ player_name($list->user_id_two) }}
                            </td>
                            <td class="px-2 py-4 text-black border-r text-center">
                               {{ player_email($list->user_id_two) }}
                            </td>
                            <td class="px-2 py-4 text-black border-r text-center">
                               {{ player_phone($list->user_id_two) }}
                            </td>
                            ...
                            @elseif($list->user_id_two == $user['id'])
                                <tr class=" bg-white  dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <td class="px-2 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap border-r text-center">
                                        {{ $loop->iteration }}
                                    </td>
                                    <td class="px-2 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap border-r text-center">
                                        {{ player_id($list->user_id_one) }}
                                    </td>

                                    <td class="px-2 py-4 text-black border-r text-center">
                                        {{ player_name($list->user_id_one) }}
                                    </td>
                                    <td class="px-2 py-4 text-black border-r text-center">
                                        {{ player_email($list->user_id_one) }}
                                    </td>
                                    <td class="px-2 py-4 text-black border-r text-center">
                                        {{ player_phone($list->user_id_one) }}
                                    </td>
                                    <td class="px-2 py-4 text-black border-r whitespace-nowrap text-center">
                                        Accepted
                                    </td>
