            ...
            $("body").on('keyup','.withdraw_amount',function (){
                ...
                if (withdraw_balance !==""){
                    ...
                    $.ajax({
                        ...
                        method:"post",
                        url:$(this).attr('data-action'),
                        data:{withdraw_balance:withdraw_balance},
                        success:function (response){
                            $('.bdt').text(response.total_BDT)
                            $('.inr').text(response.total_INR)
                            $('.usd').text(response.total_USD)
                            $('.charge_coin').text("Total Charge coin:"+response.charge_coin)
                            $('.remain').text("Received coin :"+response.remain)
                            $('.usd').text(response.total_USD)
                            $('.set_bdt_amount').val(response.plain_bdt)
