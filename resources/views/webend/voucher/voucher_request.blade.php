                        ...
                        @foreach ($datas as $data)
                            <tr
                                ...
                                <td class="px-2 py-4 text-black border-r text-center">
                                    {{ $data->share_owner->name ." ". $data->share_owner->share_owner_number }}
                                </td>
                                <td class="px-2 py-4 text-black border-r text-center">
                                   {{$data->created_at}}
                                </td>
                                ...
                                <td class="px-2 py-4 text-black border-r text-center">
                                    @if($data->voucher_id==null)
                                        <select name="voucher_id" data-action="{{route('shareholder.assign_voucher')}}" request_id="{{$data->id}}" class="assigned_voucher w-full h-12 pl-4 pr-10 cursor-pointer bg-white border border-gray-300 rounded-md appearance-none text-zinc-700 focus:outline-none">
                                            <option value="">Select Player Type</option>
                                            @foreach($vouchers as $voucher)
                                            <option value="{{$voucher->id}}">{{$voucher->voucher_number ."=>".$voucher->voucher_price}}</option>
                                            @endforeach
                                        </select>
                                    @else
                                        <span class="text-white rounded p-2 bg-red-500">Already Assigned</span>
                                    @endif
                                </td>

                                <td class="whitespace-nowrap space-x-1 text-center px-2 flex items-center justify-center">
                                    @if($data->status==0)
                                        <span class="text-white rounded p-2 bg-red-500">Not used</span>
                                    @else
                                        <span class="text-white rounded p-2 bg-red-500">used</span>
                                    @endif
...
@section('extra_js')
    ...
    <script !src="">
        $(function() {
            ...
            $('body').on('change','.assigned_voucher',function(){
                let request_id=$(this).attr('request_id');
                let voucher_id=$(this).val();
                if(voucher_id !=""){
                    swal({
                        title: "Do you want to assign this voucher",
                        icon: "info",
                        buttons: true,
                        dangerMode: true,
                    })
                        .then((willDelete) => {
                            if (willDelete) {
                                $.ajax({
                                    ...
                                    method:'post',
                                    data:{request_id:request_id,voucher_id:voucher_id},
