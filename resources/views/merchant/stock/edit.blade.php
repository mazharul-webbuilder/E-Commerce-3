        ...
        <div class="container px-2 mx-auto xl:px-5">
                ...
                <ol class="flex items-center space-x-1 md:space-x-3 flex-wrap">
                    ...
                    <li aria-current="page" class="flex items-center">
                        ...
                        <div class="flex items-center">
                            <span class="ml-1 font-medium text-gray-500 md:ml-2 dark:text-gray-400">Update Stock</span>
                        </div>
                    </li>
                </ol>
            </div>
            ...
            <div class="border border-[#8e0789] rounded-md mt-10">
                <div class="bg-[#8e0789] overflow-hidden w-full px-0">
                    <h2 class="text-2xl font-bold py-2 text-white pl-3">Update Stock</h2>
                </div>
                <!-- Category form start -->
                <form id="submit_form" data-action="{{ route('merchant.stock.update') }}" method="POST">
                    @csrf
                    <input value="{{$stock->id}}" name="stock_id" type="hidden" >
                    <div class="flex flex-col gap-4 p-4 mt-3">
                        <div class="flex flex-col md:flex-row justify-between gap-3">
                            <div class="w-5/12">
                                <div class="w-full">
                                    <h4 class="mb-2 font-medium text-zinc-700">Product title</h4>
                                    <input value="{{$stock->product->title}}" name="product_title" readonly class="w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none" type="text" >
                                </div>
                            </div>
                                ...
                                <div class="w-full">
                                    ...
                                    <select class="w-full h-12 pl-4 pr-10 cursor-pointer bg-white border border-gray-300 rounded-md appearance-none text-zinc-700 focus:outline-none"
                                        ...
                                        @foreach($sizes as $size)
                                            <option value="{{$size->id}}" {{$size->id==$stock->size_id ? 'selected' : ''}}>{{$size->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="w-5/12">
                                <div class="w-full">
                                    ...
                                    <input placeholder="Enter Quantity" value="{{$stock->quantity}}" name="quantity" min="1" class="w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none" type="number" >
                ...
                $.ajax({
                    ...
                    success:function(data){
                        if (data.response === 200)
                        {
                            ...
                            $(".submit_button").text("Update Stock").prop('disabled', false)
                        }
                    },
