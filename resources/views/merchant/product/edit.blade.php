        ...
        <div class="container px-2 mx-auto xl:px-5">
                ...
                <ol class="flex items-center space-x-1 md:space-x-3 flex-wrap">
                    ...
                    <li aria-current="page" class="flex items-center">
                        ...
                        <div class="flex items-center">
                            <span class="ml-1 font-medium text-gray-500 md:ml-2 dark:text-gray-400">Product Update</span>
                        </div>
                    </li>
                </ol>
            </div>
            ...
            <div class="border border-[#8e0789] rounded-md mt-10">
                <div class="bg-[#8e0789] overflow-hidden w-full px-0">
                    <h2 class="text-2xl font-bold py-2 text-white pl-3">Product Update</h2>
                </div>
                <!-- Category form start -->
                <form id="submit_form" data-action="{{route('merchant.product.update')}}" method="POST" enctype="multipart/form-data" >
                    @csrf
                    ...
                    <div class="flex flex-col gap-4 p-4 mt-3">
                        <div class="flex flex-col md:flex-row justify-between gap-3">
                            <div class="w-full">
                                <div class="w-full">
                                    ...
                                    <input placeholder="Product Title" value="{{$product->title}}" name="title" class="w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none" type="text" >
                            ...
                            <div class="w-full">
                                <div class="w-full">
                                    ...
                                    <textarea placeholder="Product short description" name="short_description" class="w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none">{{$product->short_description}}</textarea>
                            ...
                            <div class="w-full">
                                <div class="w-full">
                                    ...
                                    <input placeholder="Weight" min="0.01" value="{{$product->weight}}" name="weight" class="w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none" type="number" step="0.01" >
                        ...
                        <div class="flex flex-col md:flex-row justify-between gap-3">
                            ...
                            <div class="w-full">
                                ...
                                <div class="relative inline-flex w-full">
                                    <select class="w-full h-12 pl-4 pr-10 cursor-pointer bg-white border border-gray-300 rounded-md appearance-none text-zinc-700 focus:outline-none load_sub_cat" name="sub_category_id">
                            ...
                            <div class="w-full">
                                <h4 class="mb-2 font-medium text-gray">Product Brand</h4>
                                <div class="relative inline-flex w-full">
                                    <select
                                        ...
                                        @foreach ($brands as $brand)
                                            <option value="{{ $brand->id }}" {{$brand->id == $product->brand->id ? "selected" : ""}}>{{ $brand->brand_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="w-full">
                                ...
                                <div class="relative inline-flex w-full">
                                    <select class="w-full h-12 pl-4 pr-10 cursor-pointer bg-white border border-gray-300 rounded-md appearance-none text-zinc-700 focus:outline-none" name="unit_id">
                        ...
                        <div class="flex flex-col md:flex-row justify-between gap-3">
                            ...
                            <div class="w-full">
                                <div class="w-full">
                                    ...
                                    <input min="1" placeholder="Purchase Price" value="{{$product->purchase_price}}" name="purchase_price" class="w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none" type="number" >
                            ...
                            <div class="w-full">
                                <div class="w-full">
                                    ...
                                    <input min="1" placeholder="Previous Price" value="{{$product->previous_price}}" name="previous_price" class="w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none" type="number" >
                        ...
                        <div class="flex flex-col md:flex-row justify-between gap-3">
                            ...
                            <div class="w-full">
                                <div class="w-full">
                                    ...
                                    <input min="0" value="{{$product->delivery_charge_in_dhaka}}" placeholder="Delivery Charge in dhaka" name="delivery_charge_in_dhaka" class="w-full h-12 px-4 border border-gray-300 rounded-md text-zinc-700 focus:outline-none" type="number" >
                                    <span class="delivery_charge_in_dhaka_error text-red-400"></span>
                                </div>
                            </div>
                            <div class="w-full">
                        ...
                        <div class="flex flex-col md:flex-row justify-between gap-3">
                            ...
                            <div class="w-full">
                                ...
                                <textarea rows="3" cols="3" placeholder="Description" name="description" class="summernote w-full px-4 py-2 border border-gray-300 rounded-md text-zinc-700 focus:outline-none" type="text">{{$product->description}}</textarea>
        ...
        $(document).ready(function (){
            ...
            function setProductCurrentCoin()
            {
                ...
                if (commission > 100) {
                    ...
                    $('#purchaseCoin').attr('value', purchaseCoin * {{setting()->coin_per_dollar}})
                }
            }
            /*---------------------------------------------------------------*/
                ...
                $.ajax({
                    ...
                    error:function(response){
                        ...
                        if (response.status === 422) {
                            ...
                            if (response.responseJSON.errors.hasOwnProperty('short_description')) {
                                $('.short_description_error').html(response.responseJSON.errors.short_description)
                            }else{
                            ...
                            if (response.responseJSON.errors.hasOwnProperty('category_id')) {
                                $('.category_error').html(response.responseJSON.errors.category_id)
                            }else{
                                $('.category_error').html('')
                            }

                            if (response.responseJSON.errors.hasOwnProperty('sub_category_id')) {
                                $('.sub_cat_error').html(response.responseJSON.errors.sub_category_id)
                            }else{
                                $('.sub_cat_error').html('')
                            }
                            ...
                            if (response.responseJSON.errors.hasOwnProperty('thumbnail')) {
                                $('.thumbnail_error').html(response.responseJSON.errors.thumbnail)
                            }else{
