...
class AffiliatorWithdrawController extends Controller
{
    ...
    /**
     * View Withdraw List Of Affiliator
     */
    public function index(): View
    {
        return  \view('webend.ecommerce.withdraw.affiliator.index');
    }
    ...
    {
        $withdraw_lists = DB::table('withdraw_histories')->where('affiliator_id', '!=', 'null')->latest()->get();
        ...
        try {
            ...
            if ($withdraw->status > 2) {
                ...
                if ($withdraw->status == 4) {
                    $affiliator = Affiliator::find($withdraw->affiliator_id);
                    $affiliator->balance += $withdraw->withdraw_balance;
                    $affiliator->save();
                }
                return response()->json([
                    'response' => Response::HTTP_OK,
